...
library TypedMemView {
    ...
    uint256 private constant LOW_27_BYTES_MASK =
        0xffffffffffffffffffffffffffffffffffffffffffffffffffffff; // (1 << _27_BYTES_IN_BITS) - 1;
    ...
    function assertValid(bytes29 memView) internal pure returns (bytes29) {
        if (isNotValid(memView))
            revert TypedMemView__assertValid_validityAssertionFailed();
        return memView;
    }
    ...
    function assertType(
        ...
        if (!isType(memView, _expected)) {
            revert TypedMemView__assertType_typeAssertionFailed(
                uint256(typeOf(memView)),
                uint256(_expected)
            );
        }
        return memView;
    ...
    function index(
        ...
        if (_index + _bytes > len(memView)) {
            ...
            revert TypedMemView__index_overrun(
                loc(memView),
                len(memView),
                _index,
                uint256(_bytes)
    ...
    function unsafeCopyTo(
        ...
        if (isNotValid(memView))
            revert TypedMemView__unsafeCopyTo_invalidPointer();
