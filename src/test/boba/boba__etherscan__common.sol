...
contract BobaEtherscanCommon is DSCommon {
    ...
    // Governance
    address L2__BatchMessageRelayer =
        address(0x5273a421Ef862DfCeFb9144F7C90fdC6342Ad462);
    address ProxyAdmin = address(0x1f2414D0af8741Bc822dBc2f88069c2b2907a840);

    // Protocol with interface
    IL1MultiMessageRelayer L1__MultiMessageRelayer =
        IL1MultiMessageRelayer(0x5fD2CF99586B9D92f56CbaD0A3Ea4DF256A0070B);

    // Protocol with address only
    address Proxy_OVM_L1CrossDomainMessenger =
        address(0x6D4528d192dB72E282265D6092F4B872f9Dff69e);
    address L1__CrossDomainMessenger =
        address(0x12Acf6E3ca96A60fBa0BBFd14D2Fe0EB6ae47820);
    address Lib_AddressManager =
    ...
    // Liquidity Pools
    IL1ChugSplashProxy proxy__L1__StandardBridge =
        IL1ChugSplashProxy(payable(0xdc1664458d2f0B6090bEa60A8793A4E66c2F1c00));
    ...
    function setUp2() public virtual {
        ...
        cheats.label(
            address(L2__BatchMessageRelayer),
            "L2__BatchMessageRelayer"
        );
        cheats.label(address(ProxyAdmin), "ProxyAdmin");

        // Assign label to protocol
        cheats.label(
            address(L1__MultiMessageRelayer),
            "L1__MultiMessageRelayer"
        );
        cheats.label(
            address(Proxy_OVM_L1CrossDomainMessenger),
            "Proxy_OVM_L1CrossDomainMessenger"
        );
        cheats.label(
            address(L1__CrossDomainMessenger),
            "L1__CrossDomainMessenger"
        );
        ...
        // Assign label to LPs
        cheats.label(
            address(proxy__L1__StandardBridge),
            "proxy__L1__StandardBridge"
        );
